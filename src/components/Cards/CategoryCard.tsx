...
const CategoryCard: FC<CategoryCardProps> = ({ category, moduleType }) => {
  const router = useRouter();
  const activeModule = useSelector(
    (state: RootState) => state.module?.activeModule
  );
  const businessType =
    moduleType || (router.query.business_type as string) || activeModule;

  const basePath = category?.parent_slug
    ? `/categories/${category.parent_slug}?subcategory=${category.slug}`
    : `/categories/${category.slug}`;

  const link = businessType
    ? `${basePath}${basePath.includes("?") ? "&" : "?"}business_type=${businessType}`
    : basePath;
