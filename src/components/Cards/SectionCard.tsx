...
const SectionCard: React.FC<SectionCardProps> = ({ section }) => {
  ...
  return (
    <Link href={`/feature-sections/${section.slug}`} className="w-full block">
      <Card
        as="div"
        radius="lg"
        shadow="sm"
        className="group overflow-hidden hover:shadow-xl transition-all duration-300 border border-default-200 hover:border-default-300 bg-content1 cursor-pointer"
      >
