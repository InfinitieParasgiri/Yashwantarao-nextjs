...
const ProductCardAddButton: FC<ProductCardAddButtonProps> = ({
  ...
  const isRestaurant = activeModule === "restaurant";
        ...
        try {
          ...
          if (cartItem) {
            ...
            if (quantity === 0) {
              ...
              response = await removeItemFromCart(cartItem.id, business_type);
            } else {
              ...
              response = await updateCartItem({
                ...
                dark_store_id: cartItem.dark_store_id,
              });
            }
          } else {
            ...
            response = await addToCart({
              ...
              quantity: quantity,
              business_type: business_type,
              dark_store_id: business_type === "grocery" ? defaultVariant.dark_store_id : undefined,
            });
          }
  ...
  const handleQuantityChange = useCallback(
    (newQuantity: number) => {
      ...
      const minQuantity = Math.ceil(rawMin / step) * step;
      ...
      if (!isRestaurant && newQuantity > stock) {
        addToast({
  ...
  const hasAddonGroups =
    defaultVariant.addon_groups && defaultVariant.addon_groups.length > 0;

  const handleInitialAdd = () => {
    if (!isRestaurant && isSimpleProduct && !hasAddonGroups) {
      const rawMin = product.minimum_order_quantity || 1;
      const step = product.quantity_step_size || 1;
      const minQuantity = Math.ceil(rawMin / step) * step;
      handleQuantityChange(minQuantity);
    } else {
      // For restaurant products, variant products OR simple products with addons, open modal
      onOpenModal();
    }
  };
