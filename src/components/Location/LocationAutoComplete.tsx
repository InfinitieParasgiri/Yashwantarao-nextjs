...
import { cleanAddress } from "@/helpers/functionalHelpers";
...
>(({ onLocationSelect, initialLocation, placeholder, isDisabled }, ref) => {
      ...
      if (selectedItem) {
        ...
        try {
          ...
          if (result?.results[0]?.geometry?.location) {
            ...
            const cleanLabel = cleanAddress(selectedItem.label);
            onLocationSelect({
              placeName: cleanLabel,
              latLng: { lat, lng },
            ...
            const displayText = selectedItem.description
              ? `${cleanLabel}, ${selectedItem.description}`
              : cleanLabel;
            setInputValue(displayText);
          }
        } catch (error) {
  ...
  return (
    <div className="w-full">
      <Autocomplete
        ...
        items={predictions}
        placeholder={placeholder || t("enter-city-or-address")}
        variant="faded"
        ...
        endContent={
          <button
            ...
            disabled={gettingCurrentLocation || isDisabled}
            className="p-1 rounded-full hover:bg-default-200 focus:outline-none focus:ring-2 focus:ring-default-400 cursor-pointer disabled:opacity-50 disabled:pointer-events-none"
