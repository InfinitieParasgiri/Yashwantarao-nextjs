...
import { handleCheckZone, cleanAddress } from "@/helpers/functionalHelpers";
...
const LocationSelector = ({ variant = "default" }: { variant?: "default" | "white" }) => {
    ...
    const initializeLocation = () => {
      try {
        ...
        if (userLocation && userLocation.lat && userLocation.lng) {
          const locationData = {
            placeName: cleanAddress(userLocation.placeName) || "Selected Location",
  ...
  const handleMapLocationUpdate = useCallback(
    async (
      ...
      try {
        ...
        if (result?.results[0]) {
          const newLocation = {
            placeName: cleanAddress(result.results[0].formatted_address),
            latLng: latLng,
            placeDescription: "",
          };
      ...
      try {
        ...
        if (res) {
          ...
          const userLocation: UserLocation = {
            lat: finalLatLng.lat,
            lng: finalLatLng.lng,
            placeName: cleanAddress(finalLocation.placeName),
  ...
  const getButtonText = () => {
    ...
    if (selectedLocation) {
      const cleaned = cleanAddress(selectedLocation.placeName);
      const displayText = selectedLocation.placeDescription
        ? `${cleaned}, ${selectedLocation.placeDescription}`
        : cleaned;
  ...
  return (
    <div>
      ...
      <Button
        ...
        onPress={onOpen}
        className={clsx(
          "p-0 py-0 px-4 h-10 max-w-full transition-all duration-300",
          variant === "white" 
            ? "bg-white text-gray-500 rounded-full shadow-sm hover:shadow-md border-none" 
            : "bg-transparent text-foreground"
        )}
        startContent={<MapPin width={16} className={variant === "white" ? "text-primary-500" : ""} />}
        endContent={<ChevronDown width={16} className={variant === "white" ? "text-gray-400" : ""} />}
        isDisabled={!isInitialized}
        fullWidth
      >
        <span className={clsx("truncate text-left flex-1 text-sm font-medium", variant === "white" ? "text-gray-400" : "")}>{getButtonText()}</span>
      </Button>
