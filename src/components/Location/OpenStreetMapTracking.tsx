...
const createCustomIcon = (
  ...
  const size = iconType === "rider" ? (isCourier ? 32 : 56) : iconType === "customer" ? 35 : 32;

  if (iconType === "rider") {
    const imgSrc = isCourier ? "/assets/courier_boy_mini.png" : "/images/delivery-boy-top-view.png";
    const rotationStyle = isCourier ? "" : `transform: rotate(${rotation}deg); transition: transform 0.2s ease-out;`;
    const shadowStyle = isCourier ? "" : "filter: drop-shadow(0 4px 10px rgba(0,0,0,0.45));";
...
const OpenStreetMapTracking: FC<OpenStreetMapTrackingProps> = ({
  ...
  useEffect(() => {
    ...
    const initializeMap = async () => {
      ...
      try {
        ...
        const hasValidRiderLocation = lat !== 0 || lng !== 0;
        ...
        stores.forEach((store, idx) => {
          ...
          let iconChar = isCollected ? "✓" : `${idx + 1}`;
          let popupText = `<div style="padding: 8px;"><strong>Store ${idx + 1} ${isCollected ? "(Collected)" : ""}</strong><p>${store.store_name}</p></div>`;
          let tooltipText = `Store ${idx + 1}`;

          if (isCourier) {
            const packageSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/></svg>`;
            iconChar = isCollected ? "✓" : packageSvg;
            popupText = `<div style="padding: 8px;"><strong>Pickup Location ${isCollected ? "(Picked Up)" : ""}</strong><p>${store.address || store.store_name}</p></div>`;
            tooltipText = `Pickup Location`;
          }

          m.setIcon(createCustomIcon(isCollected ? "#9CA3AF" : "#3B82F6", iconChar, "store"));
          m.setOpacity(isCollected ? 0.7 : 1);
          m.bindPopup(popupText);
          m.bindTooltip(tooltipText, { direction: 'top', offset: [0, -32], className: 'font-semibold' });
        });
        ...
        if (!customerMarkerRef.current) {
          let customerIconChar = "🏠";
          let customerPopupText = `<div style="padding: 8px;"><strong>Delivery Location</strong><p>${data.order.shipping_address_1}</p></div>`;
          let tooltipText = `Delivery Location`;

          if (isCourier) {
            const dotSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="5"/></svg>`;
            customerIconChar = dotSvg;
            customerPopupText = `<div style="padding: 8px;"><strong>Drop Location</strong><p>${data.order.shipping_address_1}</p></div>`;
            tooltipText = `Drop Location`;
          }

          customerMarkerRef.current = L.marker(cPos, { icon: createCustomIcon("#DC2626", customerIconChar, "customer") }).addTo(map);
          markersRef.current.push(customerMarkerRef.current);
          customerMarkerRef.current.bindPopup(customerPopupText);
          customerMarkerRef.current.bindTooltip(tooltipText, { direction: 'top', offset: [0, -32], className: 'font-semibold' });
        } else {
  ...
  useEffect(() => {
    if (!isDeliveryBoyAssigned || !mapRef.current) return;
    ...
    if (!riderMarkerRef.current) {
      if (lat !== 0 || lng !== 0) {
        riderMarkerRef.current = L.marker([lat, lng], {
          icon: createCustomIcon("#10B981", "🏍️", "rider", 0, isCourier),
          zIndexOffset: 1000,
      ...
      try {
        ...
        const animate = (time: number) => {
          ...
          if (riderMarkerRef.current) {
            ...
            if (icon && !isCourier) icon.style.transform = `rotate(${currHeading}deg)`;
