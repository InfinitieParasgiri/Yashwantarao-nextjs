...
const searchFetcher = async (
  ...
  const [, query, activeModule] = key.split(":");
...
const GlobalSearchBar: React.FC = () => {
  ...
  const swrKey = debouncedSearchQuery.trim()
    ? `search:${debouncedSearchQuery}:${activeModule}`
    : null;
  ...
  const handleSearchSubmit = useCallback(
    (query: string = searchQuery) => {
      ...
      if (trimmedQuery.length >= 2) {
        ...
        if (trimmedQuery !== debouncedSearchQuery) {
          mutate(searchFetcher(`search:${trimmedQuery}:${activeModule}`), false);
        }
        onClose();
        router.push({
    ...
    [searchQuery, addSearch, debouncedSearchQuery, mutate, router, onClose, activeModule],
  );
