...
export default function FirebaseInitializer({
  ...
  useEffect(() => {
    try {
      ...
      if (firebaseConfig && !firebase) {
        ...
        if (typeof window !== "undefined") {
          try {
            ...
            const errorMsg = `Failed to configure Firebase Auth: ${authError instanceof Error ? authError.message : "Unknown error"
              }`;
            console.error(errorMsg);
            addToast({
      ...
      const errorMsg = `Error processing Firebase initialization: ${error instanceof Error ? error.message : "Unknown error"
        }`;
      console.error(errorMsg);
      addToast({
