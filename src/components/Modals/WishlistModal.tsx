...
interface WishlistModalProps {
  ...
  onWishlistUpdate?: (wishlistId: string, action: "add" | "remove") => void;
}
...
const WishlistModal = ({
  ...
  const handleSaveToSelectedWishlist = async () => {
    ...
    try {
      ...
      if (response.success) {
        onWishlistUpdate?.(selectedWishlistId, "add");
        triggerRefetchButtons();
        await router.push(
          {
            pathname: router.pathname,
  ...
  const handleConfirmDeleteWishlist = async () => {
    ...
    try {
      const response = await deleteWishlistById(id.toString(), business_type);
      if (response.success) {
        onWishlistUpdate?.(id.toString(), "remove");
        await router.push(
          {
            pathname: router.pathname,
            query: router.query,
  ...
  const handleConfirmRemoveItem = async () => {
    ...
    try {
      const response = await deleteWishlistItemById(favoriteItem.id, business_type);
      if (response.success) {
        onWishlistUpdate?.(wishlistItemToRemove.id.toString(), "remove");
        triggerRefetchButtons();
        await router.push(
