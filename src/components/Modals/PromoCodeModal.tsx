...
const fetcher = async ([url, business_type]: [string, string | undefined]): Promise<PromoCode[]> => {
  const response: ApiResponse<PromoCode[]> = await getPromoCodes(business_type);
...
const PromoCodeModal: React.FC<PromoCodeModalProps> = ({
  ...
  const {
    ...
    isValidating,
    mutate,
  } = useSWR<PromoCode[]>(isOpen ? ["/promo-codes", business_type] : null, fetcher, {
