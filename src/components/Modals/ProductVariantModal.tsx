...
const ProductVariantModal: FC<ProductVariantModalProps> = ({
  ...
  const minQuantity = Math.ceil(
    (Number(product.minimum_order_quantity) || 1) /
      (Number(product.quantity_step_size) || 1)
  ) * (Number(product.quantity_step_size) || 1);
  ...
  useEffect(() => {
    ...
    setQuantity(editingQuantity ?? minQuantity);
    setLightboxOpen(false);

    const initialQtys: Record<number, number> = {};
    product.variants.forEach((v) => {
      initialQtys[v.id] = minQuantity;
    });
  ...
  const handleAddToCartFn = async (
    ...
    try {
      if (editingCartItemId) {
        if (isLoggedIn) {
          const cartItemToEdit = cartData?.items?.find((i) => i.id === editingCartItemId);
          await handleUpdateCartItem({
            cartItemId: editingCartItemId,
            quantity: qty,
            addons: addonsForApi,
            dark_store_id: cartItemToEdit?.dark_store_id,
            onClose,
            renderToast: true,
          });
        } else {
        ...
        await handleAddToCart({
          ...
          renderToast: true,
          addons: addonsForApi,
          dark_store_id: activeModule === "grocery" ? v.dark_store_id : undefined,
        });
      } else {
        handleOfflineAddToCart({
          product,
                            ...
                            {group.items.slice(0, 5).map((item) => {
                              ...
                              return (
                                <Radio
                                  ...
                                  classNames={{
                                    base: `max-w-full m-0 flex-row-reverse w-full px-3 py-2.5 items-center justify-between gap-3 transition-colors ${isDisabled
                                ...
                                >
                                  <div className="flex items-center gap-3">
                                    {item.indicator ? (
                                      <span
                                        className={`shrink-0 w-3.5 h-3.5 rounded-sm border-2 flex items-center justify-center ${item.indicator === "veg"
                                            ? "border-green-600"
                                            : "border-red-600"
                                          }`}
                                      >
                                        <span
                                          className={`w-2 h-2 rounded-full ${item.indicator === "veg"
                                              ? "bg-green-600"
                                              : "bg-red-600"
                                            }`}
                                        />
                    ...
                    {product.variants.map((v) => {
                      ...
                      return (
                        <div
                          ...
                          className={`border rounded-lg p-3 space-y-3 transition-all ${hasAddonGroups
                              ? isSelected
                              ...
                              {product.store_status.is_open ? (
                                ...
                                >
                                  {v.stock === 0
                                    ...
                                    : `${t("product_modal.add_to_cart_title")} • ${currencySymbol +
                                    (price * qty).toFixed(2)
                                    }`}
                                </Button>
                              ) : (
                  ...
                  <div className="flex items-end justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      <Button
                        ...
                        onPress={() =>
                          setQuantity((q) =>
                            Math.max(minQuantity, q - stepSize),
                          )
                        }
