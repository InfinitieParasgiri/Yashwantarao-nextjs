...
import { getCourierDeliveryBoyLocation } from "@/routes/api";
import dynamic from "next/dynamic";
const OpenStreetMapTracking = dynamic(
  () => import("@/components/Location/OpenStreetMapTracking"),
  { ssr: false }
);
import {
  MapPin,
...
interface TrackCourierModalProps {
  isOpen: boolean;
  onClose: () => void;
  courierRequest: CourierRequest;
}

const TrackCourierModal: FC<TrackCourierModalProps> = ({
  isOpen,
  onClose,
  courierRequest,
}) => {
  ...
  const fetchDeliveryBoyLocation = useCallback(async () => {
    ...
    try {
      const res: ApiResponse<DeliveryLocationResponse> =
        await getCourierDeliveryBoyLocation(courierRequest.id);
          ...
          <div className="flex items-center gap-2">
            <div>
              <div className="flex gap-2 justify-start items-center">
                <h2 className="text-lg font-semibold">{t("track", "Track Live")}</h2>
                <Button
                  isIconOnly
                  size="sm"
                  variant="light"
                ...
                <Card>
                  <CardHeader className="flex justify-between items-start">
                    <div className="flex items-center gap-2">
                      ...
                      <p className="font-medium text-sm">
                        {t("trackOrderModal.deliveryRoute", "Courier Route")}
                      </p>
                    </div>
                  ...
                  <CardBody className="p-3 space-y-0">
                    ...
                    <div className="relative pl-8">
                      ...
                      <p className="font-medium text-xs">
                        {t("drop_location", "Drop-off Location")}
                      </p>
                      <p className="text-gray-500 text-[11px]">
