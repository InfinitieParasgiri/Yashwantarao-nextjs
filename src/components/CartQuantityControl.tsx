...
import { updateCartItem } from "@/routes/api";
...
const CartQuantityControl: React.FC<CartQuantityControlProps> = ({
  ...
  const debouncedUpdateQuantity = useMemo(
      ...
      debounce(async (itemId: string | number, quantity: number) => {
        ...
        try {
          const response = await updateCartItem({
            ...
            dark_store_id: item.dark_store_id,
          });
    ...
    [item, debounceDelay, dispatch, t, business_type]
  );

  const parsedMin = Number(minQuantity) || 1;
  const parsedStep = Number(quantityStep) || 1;
  const validMinQuantity = Math.ceil(parsedMin / parsedStep) * parsedStep;

  const handleQuantityChange = useCallback(
    (newQuantity: number) => {
      if (newQuantity < validMinQuantity) {
        addToast({
          title: t("min_quantity_error_title"),
          description: t("min_quantity_error_description", {
            min: validMinQuantity,
          }),
          color: "danger",
        });
        return;
      ...
      const applicableStock = activeModule === "restaurant" ? 9999 : stock;
      if (newQuantity > applicableStock) {
        addToast({
          title: t("stock_limit_error_title"),
          description: t("stock_limit_error_description", { stock: applicableStock }),
          color: "danger",
        });
        return;
      }

      if ((newQuantity - validMinQuantity) % parsedStep !== 0) {
        addToast({
          title: t("step_error_title"),
          description: t("step_error_description", { step: parsedStep }),
          color: "danger",
        });
        return;
      }
  ...
  const handleIncrement = useCallback(
    () => handleQuantityChange(localQuantity + parsedStep),
  ...
  return (
    <div className="flex items-center gap-2 md:gap-4">
      <button
        className={`p-0.5 bg-slate-100 dark:bg-gray-700 rounded-full transition ${isUpdating
