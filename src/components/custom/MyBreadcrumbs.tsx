...
const MyBreadcrumbs: React.FC<MyBreadcrumbsProps> = ({
  ...
  const routeHome = useHomeRoute();

  const finalHomeHref = homeHref ?? routeHome;
  ...
  return (
    ...
    >
      {allBreadcrumbs.map((breadcrumb, index) => {
        // We use Next.js Link to ensure client-side routing without full page reload.
        // The 'as' prop might not be perfectly typed for all NextUI/HeroUI versions,
        // so we cast to any or explicitly use Next Link if needed.
        return (
          <BreadcrumbItem
            key={index}
