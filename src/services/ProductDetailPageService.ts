...
export async function fetchProductDetailPageData(
  ...
  const { slug, access_token, lat = "", lng = "", PER_PAGE = 20, business_type } = params;

  // Concurrent API calls
  const [productDetailResult, similarProductsResult, settingsResult] =
    await Promise.allSettled([
      getProductBySlug({ slug, access_token, latitude: lat, longitude: lng, business_type }),
      getProducts({
  ...
  return {
    initialProduct:
      productDetailResult.status === "fulfilled" && productDetailResult.value.success
        ? (Array.isArray(productDetailResult.value.data) ? productDetailResult.value.data[0] : (productDetailResult.value.data ?? null))
        : null,
