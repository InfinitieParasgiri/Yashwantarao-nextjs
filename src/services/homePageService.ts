...
type HomePageParams = {
  ...
  access_token?: string;
  homeCategory?: string;
  business_type?: "grocery" | "restaurant" | "courier";
};
...
export const getHomePageData = async (
  ...
  const { lat = "", lng = "", access_token = "", homeCategory = "", business_type } = params;
  ...
  if (lat && lng) {
    ...
    try {
      const isCourier = homeCategory === "courier";
      const bannersResponse = await getBannerImages({
        scope_category_slug: isCourier ? undefined : (homeCategory ? homeCategory : undefined),
        business_type: business_type || (isCourier ? "courier" : undefined),
        per_page: 50,
        latitude: lat,
        longitude: lng,
      });
    ...
    // Fetch stores
    try {
      const storesResponse = await getStores({ latitude: lat, longitude: lng, business_type });
