...
const DeliveryBanner: React.FC = () => {
  ...
  const isCourierModule = router.pathname.startsWith("/courier");

  let targetPath = "/brands";
  if (isCourierModule) {
    targetPath = "/courier/book";
  } else if (activeModule === "restaurant") {
    targetPath = "/stores";
  }

  const buttonLabel = isCourierModule ? t("courier.bookNow", "Book Now") : t("home.delivery.button");
