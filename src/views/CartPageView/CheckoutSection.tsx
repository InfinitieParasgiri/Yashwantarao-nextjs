...
const CheckoutSection: FC<CheckoutSectionProps> = ({ cart }) => {
  ...
  const [isWalletUse, setIsWalletUse] = useState(payment_summary?.use_wallet ?? false);
  ...
  const handleCheckoutClick = async (walletOnly = false) => {
    try {
      ...
      // Check closed stores (skip for grocery as it uses dark stores, not regular stores)
      const closedStores =
        items?.length > 0 && activeModule !== "grocery"
  ...
  // Guard: payment_summary may be temporarily undefined while cart transitions between states
  if (!payment_summary) return null;
  ...
  return (
    <div className="w-full flex justify-end">
          ...
          <div className="space-y-3 pt-4 w-full text-sm">
            ...
            {selectedAddress?.id && (
              <div className="flex justify-between">
                ...
                ) : payment_summary.total_delivery_charges === 0 ? (
                  <span className="text-green-600 font-semibold">
                    {t("checkout.freeShipping", "FREE")}
                  </span>
                ) : (
            ...
            {payment_summary.per_store_drop_off_fee > 0 && (
              <div className="flex justify-between">
                <div>
                  ...
                  {activeModule !== "grocery" && !isSingleVendor && (
                    <>
      ...
      <ConfirmationModal
        ...
        onConfirm={async () => {
          const business_type = activeModule || "restaurant";
          const res = await clearCart(business_type);
          if (res?.success) {
            ...
            addToast({
              title: t("general.error.title", { defaultValue: "Error" }),
              description: res?.message || t("general.error.somethingWentWrong"),
              color: "danger",
            });
          }
        }}
