...
const CartItems: FC<CartItemsProps> = ({ items = [], currencySymbol = "" }) => {
  ...
  const handleRemoveItem = async () => {
    ...
    try {
      const response = await removeItemFromCart(selectedItemId, business_type);
      if (response.success) {
        addToast({
  ...
  const handleSaveForLater = async (itemId: number, quantity: number) => {
    try {
      const res = await saveCartItemToSaveForLater(itemId, quantity, business_type);
      if (res.success) {
        // Invalidate SaveForLaterItems SWR cache — key is ["/save-for-later", business_type]
        mutate((key) => Array.isArray(key) && key[0] === "/save-for-later", undefined, { revalidate: true });
    ...
    <ScrollShadow className="w-full max-h-[50vh] py-1 flex flex-col gap-6">
      {groupedItems.length > 0 &&
        groupedItems.map((group) => (
          ...
          >
            {/* Store Header */}
            {!isSingleVendor && business_type !== "grocery" && (
