...
const SaveForLaterItems: FC<{ moreProductsInline: boolean }> = ({
  ...
  const swrKey = isLoggedIn ? ["/save-for-later", business_type] : null;

  const {
    data: itemsData,
    isLoading: isLoadingItems,
    mutate,
  } = useSWR(swrKey, () => getSaveForLaterItems(business_type), {
  ...
  const handleMoveToCart = async (item: CartItem) => {
    ...
    try {
      ...
      if (res && typeof res !== "string" && "success" in res && "data" in res) {
        ...
        // Refresh save-for-later list — handleAddToCart already refreshes the cart
        mutate();
      } else {
        addToast({
