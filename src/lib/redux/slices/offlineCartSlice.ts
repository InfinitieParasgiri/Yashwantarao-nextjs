...
const clampQuantity = (item: OfflineCartItem, desiredQuantity: number) => {
  const rawMin = Number(item.minQuantity) || 1;
  const stepSize = Number(item.stepSize) || 1;
  const minQuantity = Math.ceil(rawMin / stepSize) * stepSize;
  const maxQuantity = Number(item.maxQuantity) || Number.MAX_SAFE_INTEGER;
  const isRestaurant = item.business_type === "restaurant";
  const stock = isRestaurant ? Number.MAX_SAFE_INTEGER : (Number(item.stock) || Number.MAX_SAFE_INTEGER);
