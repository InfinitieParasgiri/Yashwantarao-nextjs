...
import { getPageFromUrl, isSSR, getCookieFromContext } from "@/helpers/getters";
import UserLayout from "@/layouts/UserLayout";
import { getAddresses, getSettings, checkDeliveryZone } from "@/routes/api";
...
const addressesFetcher = async (url: string) => {
  const [, page, zoneId] = url.split(":");
  const response: PaginatedResponse<Address[]> = await getAddresses({
    page: parseInt(page),
    per_page,
    zone_id: zoneId && zoneId !== "null" ? parseInt(zoneId) : undefined,
  });
...
const AddressesPage: NextPageWithLayout<AddressesPageProps> = ({
  ...
  // Client-side effect to fetch current zone based on userLocation cookie
  useEffect(() => {
    const resolveZone = async () => {
      const location = getCookie("userLocation") as UserLocation | undefined;
      const activeModuleStr = store.getState().module?.activeModule;
      const business_type = activeModuleStr === "courier" ? undefined : activeModuleStr;

      if (location?.lat && location?.lng) {
        const res = await checkDeliveryZone({ latitude: location.lat, longitude: location.lng, business_type });
        if (res.success && res.data?.zone_id) {
  ...
  const swrKey = `addresses:${currentPage}:${zoneId}`;
  const {
    data: paginatedData,
    error: swrError,
    isLoading,
      ...
      <UserLayout activeTab="addresses">
        <div className="w-full">
          <div className="flex justify-between items-center">
            <PageHeader
              title={t("pages.addresses.myAddresses")}
              subtitle={`${t("pages.addresses.subtitle")}${totalAddresses > 0
...
export const getServerSideProps: GetServerSideProps | undefined = isSSR()
  ? async (context) => {
    try {
      ...
      // Resolve active delivery zone ID on the server side using the userLocation cookie if present
      let zoneId: number | undefined = undefined;
      const location = await getUserLocationFromContext(context);
      const activeModuleStr = (getCookieFromContext(context, "activeModule") as string) || undefined;
      const business_type = activeModuleStr === "courier" ? undefined : activeModuleStr;

      if (location?.lat && location?.lng) {
        const zoneRes = await checkDeliveryZone({ latitude: location.lat, longitude: location.lng, business_type });
        if (zoneRes.success && zoneRes.data?.zone_id) {
          zoneId = zoneRes.data.zone_id;
        }
      }

      const response = await getAddresses({ access_token, page, per_page, zone_id: zoneId });
