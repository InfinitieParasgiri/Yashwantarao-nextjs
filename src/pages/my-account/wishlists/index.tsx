...
const WishlistsPage: NextPageWithLayout<WishlistsPageProps> = ({
  ...
  const business_type = activeModule === "courier" ? "restaurant" : activeModule;
  ...
  const { data: wishlistsData, error: wishlistsError } = useSWR(
    ...
    async () => {
      const response = await getAllWishlistTitles({ business_type });
  ...
  const fetchWishlistDetails = async (id: string, forceFetch?: boolean) => {
    ...
    try {
      const response = await getWishlistById(id, business_type);
      if (response.success && response.data) {
  ...
  const handleDeleteWishlist = async (id: string) => {
    ...
    try {
      const response = await deleteWishlistById(id, business_type);
      if (response.success) {
        await fetchWishlists();
  ...
  const handleRemoveItem = async (itemId: number, forceFetch: boolean) => {
    ...
    try {
      const response = await deleteWishlistItemById(itemId, business_type);
      if (response.success) {
        if (selectedWishlist) {
...
export const getServerSideProps: GetServerSideProps | undefined = isSSR()
  ? async (context) => {
      try {
        ...
        const activeModule = (getCookieFromContext(context, "activeModule") as string) || "restaurant";
        ...
        const business_type = activeModule === "courier" ? "restaurant" : activeModule;
        const wishlistsResponse = await getAllWishlistTitles({ access_token, business_type });
