...
const ordersFetcher = async (url: string) => {
  ...
  const business_type = urlParams.get("business_type") || undefined;
...
const OrdersPage: NextPageWithLayout<OrdersPageProps> = ({
  ...
  const reduxActiveModule = useSelector((state: RootState) => state.module?.activeModule);
  const activeModule = reduxActiveModule || "restaurant";

  const businessType = activeModule === "courier" ? undefined : activeModule;
  ...
  let orders = swrOrders || initialOrders;
  const error = swrError ? swrError.message : initialError;

  // Filter orders by active module in case the backend returns all orders or corrupted business_type
  if (orders && orders.data && businessType) {
    const filteredData = orders.data.filter((order: Order) => {
      // Restaurant orders will have a valid store_id in their items
      const hasRestaurantStore = order.items && order.items.some((item) => !!item.store_id || !!item.store);

      if (businessType === "restaurant") {
        return hasRestaurantStore;
    ...
    orders = { ...orders, data: filteredData };
  }
...
export const getServerSideProps: GetServerSideProps | undefined = isSSR()
  ? async (context) => {
    try {
      ...
      if (!access_token) {
        const activeModule = (getCookieFromContext(context, "homeCategory") as string) || undefined;
      ...
      const activeModule = (getCookieFromContext(context, "homeCategory") as string) || "restaurant";
      const businessType = activeModule === "courier" ? undefined : activeModule;
