...
import { getSlugFromContext, isSSR, getCookieFromContext } from "@/helpers/getters";
...
const fetcher = async (slug: string, business_type?: string) => {
...
const similarProductsFetcher = async (slug: string, business_type?: string) => {
  const { lat = "", lng = "" } = getCookie("userLocation") as UserLocation;
...
const ProductPage: NextPageWithLayout<ProductPageProps> = ({
  ...
  const {
    ...
    () => fetcher(productSlug!, business_type),
    {
  ...
  const {
    ...
    () => similarProductsFetcher(productSlug!, business_type),
    {
  ...
  ? async (context) => {
      try {
        ...
        const data = await fetchProductDetailPageData({
          ...
          lng,
          PER_PAGE,
          business_type: (getCookieFromContext(context, "activeModule") as string) || undefined,
        });
