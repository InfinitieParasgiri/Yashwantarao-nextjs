...
export default function DefaultLayout({
  ...
  // Sync active module to a cookie so SSR calls during shallow routing know what module we're on
  const activeModule = useSelector((state: RootState) => state.module?.activeModule);
  useEffect(() => {
    if (activeModule) {
      setCookie("activeModule", activeModule);

      const isLoggedIn = store.getState().auth.isLoggedIn;
      if (isLoggedIn) {
        // Silently fetch updated cart data for the newly active module
        updateCartData(false, false, 0, false, false);
      }
    }
  }, [activeModule]);
